<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-indigo-600 shadow-md rounded-md">
        <h2 class="text-center text-lg text-white font-bold pt-6 tracking-widest">ホテル情報変更</h2>

        <x-validation-errors :errors="$errors" />

        <form action="{{ route('hotels.update') }}" method="POST" class="rounded pt-3 pb-8 mb-4">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <input type="text" name="name"
                    ...
                    required placeholder="名前" value="{{ old('name', $hotel->name) }}">
            </div>
            <div>
                ...
                <select name="prefecture_id"
                    ...
                    @foreach ($prefectures as $prefecture)
                        <option value="{{ $prefecture->id }}" @if ($prefecture->id == old('prefecture_id', $hotel->prefecture_id)) selected @endif>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="adress"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="住所" value="{{ old('adress', $hotel->adress) }}">
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="phonenumber"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="電話番号" value="{{ old('phonenumber', $hotel->phonenumber) }}">
            </div>
            <div class="mb-4">
                ...
                <input type="number" name="capacity"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="10部屋 => 10と記入" value="{{ old('capacity', $hotel->capacity) }}">
            </div>
            <div class="mb-4">
                ...
                <textarea name="description" rows="10"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    placeholder="詳細">{{ old('description', $hotel->description) }}</textarea>
            </div>
