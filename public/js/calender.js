        ...
        if (dateCalender.getDate() < today.getDate() && dateCalender.getMonth() <= today.getMonth() && dateCalender.getFullYear() <= today.getFullYear()) {
            ...
            if (infos[dateCalender.toLocaleDateString('sv-SE')]['opening'] === '☓') {
                ...
                if (dateCalender.getDay() === dayOfWeekSunday) {
                    ...
                    htmlCalender = htmlCalender + `<div class= "cursor-pointer cols-1 border-r clickDate h-14 text-center" data-date='${dateCalender.toLocaleDateString('sv-SE')}'><div>${dateCalender.getDate()}</div><div>${infos[dateCalender.toLocaleDateString('sv-SE')]['opening']}</div></div>`
                }
            }
        }
